...
for i=1:length(count)
    if i~=length(count)
    [A_r_gsm, A_theta_gsm, A_phi_gsm] = design_SH(r((i-1)*step+1:i*step), theta((i-1)*step+1:i*step),phi((i-1)*step+1:i*step),maxD);
    X=[A_r_gsm; A_theta_gsm; A_phi_gsm];
    d=d+X'*reshape(b((i-1)*step+1:i*step,:),[],1);
    G=G+X'*X;
    else
    [A_r_gsm, A_theta_gsm, A_phi_gsm] = design_SH(r((i-1)*step+1:length(r)), theta((i-1)*step+1:length(r)),phi((i-1)*step+1:length(r)),maxD;
    X=[A_r_gsm; A_theta_gsm; A_phi_gsm];
